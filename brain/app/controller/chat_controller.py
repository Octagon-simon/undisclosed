...
async def approval(id: str, data: ApprovalDecision, request: Request):
    ...
    event. Wakes the blocked tool executor with approved/denied."""
    chat_logger.info(
    ...
    if task_lock is None:
        raise UserException(
            code.error,
            "This task is no longer running; nothing to approve.",
        )
    manager = getattr(task_lock, "approval_manager", None)
    if manager is None:
        raise UserException(
            code.error,
            "This task has no pending approvals.",
        )
    try:
        ...
        raise UserException(code.error, str(exc)) from exc
    if not resolved:
        # Unknown or already-settled approval id: idempotent no-op.
        chat_logger.info(
            "Approval decision ignored (unknown or already resolved)",
            extra={"task_id": id, "approval_id": data.approval_id},
        )
    return Response(status_code=204)
