...
# Overall hard cap for model validation. The underlying provider calls can
# otherwise stall far longer than intended (each stage may retry internally),
# leaving the request "pending" for a very long time.
MODEL_VALIDATION_TIMEOUT_SECONDS = 90.0
...
async def validate_model(request: ValidateModelRequest):
    ...
    try:
        ...
        # The overall validation budget was exceeded. Return a clear error
        ...
        logger.error(
            ...
            extra={
                "platform": platform,
                "model_type": model_type,
                "timeout_seconds": MODEL_VALIDATION_TIMEOUT_SECONDS,
            },
            exc_info=True,
        )
        raise HTTPException(
            ...
            detail={
                "message": (
                    "Model validation timed out after "
                    f"{MODEL_VALIDATION_TIMEOUT_SECONDS:.0f}s. "
                    "The provider is not responding quickly enough. "
                    "Please check your API key, endpoint URL, network "
                    "connectivity, and model availability, then try again."
                ),
                ...
                "error": {
                    ...
                    "code": "timeout_error",
                    "message": (
                        f"Model validation timed out after "
                        f"{MODEL_VALIDATION_TIMEOUT_SECONDS:.0f}s."
                    ),
                },
            },
