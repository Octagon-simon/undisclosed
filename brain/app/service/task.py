...
class Action(str, Enum):
    ...
    acknowledge = "acknowledge"  # backend -> user (one-time turn-start ack)
    ...
    approval_request = "approval_request"  # backend -> user
    approval_resolved = "approval_resolved"  # backend -> user
...
class ActionAcknowledgeData(BaseModel):
    """A one-time, friendly acknowledgement emitted at the start of a turn to
    convey presence. Generated by a throwaway model call decoupled from the
    agent's own message history, so it never becomes part of the answer or gets
    re-fed to the model (see AGENT_ACK in single_agent_service)."""

    action: Literal[Action.acknowledge] = Action.acknowledge
    process_task_id: str
    data: str  # the full acknowledgement sentence
...
class ApprovalRequestPayload(BaseModel):
    """Data pushed to the SSE stream as `approval_request` while a tool
    execution waits for an explicit human decision."""
...
class ApprovalResolvedPayload(BaseModel):
    """Data pushed to the SSE stream as `approval_resolved` after a
    decision (or timeout) lands for a previously requested approval."""
...
class ActionApprovalRequestData(BaseModel):
    action: Literal[Action.approval_request] = Action.approval_request
    data: ApprovalRequestPayload


class ActionApprovalResolvedData(BaseModel):
    action: Literal[Action.approval_resolved] = Action.approval_resolved
...
class TaskLock:
    ...
    """Per-session ApprovalManager gating risky tool executions (governance
    ASK mode). None until the first solve loop for this task lock creates
    one."""
