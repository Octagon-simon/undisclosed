...
# Compaction tier knobs (§ context-compaction design).
#
# Tier 0: the model's own prior assistant turns are full prose that we should
# NOT resend verbatim on every request. We collapse each to a single-line gist:
# the first sentence (caption) of the reply, hard-capped so one oversized
# output (an HTML report, a CSV dump) can't blow up the context. User turns
# carry the actual intent and are preserved verbatim -- that's the signal the
# agent genuinely needs to keep continuity.

_ASSISTANT_GIST_MAX_CHARS = 240


def _first_sentence(text: str) -> str:
    """First sentence, collapsed to a single line. Falls back to whole text."""
...
def _gist_assistant_turn(content: str) -> str:
    """Compact a prior assistant turn to the token-minimum that keeps its gist.

    Keeps the leading caption as a thin pointer to the original (which lives in
    the audit trail / full task log) rather than re-injecting the whole body.
    The `[see full result]` pointer is appended on EVERY gisted turn -- even a
    caption that fits under the cap -- so the agent never mistakes the short
    caption for the entirety of the prior reply. The full original always lives
    in the audit trail.
    """
    gist = _first_sentence(content)
    if not gist:
        return ""
    if len(gist) <= _ASSISTANT_GIST_MAX_CHARS:
        return gist + " [see full result]"
    return _truncate(gist, _ASSISTANT_GIST_MAX_CHARS, " ... [see full result]")


# Tier 1: a "Known facts" bullet whose text is already present verbatim in the
# kept conversation is pure duplication -- the conversation line *is* the thin
# pointer. We skip that fact bullet for the context view. Only genuinely novel
# facts (not already spelled out in the visible turns) earn their own bullet.

def _fact_is_redundant(
    fact: "MemoryFact",
    visible_conversation: list["ConversationEvent"],
) -> bool:
    """True when `fact.text` already appears inside the kept conversation turns."""
    needle = " ".join((fact.text or "").split())
    if not needle:
        return True
    for event in visible_conversation:
        haystack = " ".join((event.content or "").split())
...
class AgentContextBundle:
    ...
    def _render_single_agent(self) -> str:
        ...
        #
        # Compaction tiers (§ context-compaction design):
        #   Tier 2 — split into a STABLE prefix (Space + Project + facts +
        #   artifacts + todos, which don't change for a given project) followed
        #   by a VOLATILE delta (recent conversation + current-turn instruction,
        #   which grow every request). The stable prefix is byte-identical
        #   across requests for the same project, so the model-provider prompt
        #   cache can collapse and re-bill it at a fraction.
        #   Tier 1 — skip "Known facts" bullets whose text is already present
        #   verbatim in the visible conversation (that line *is* the pointer).
        #   Tier 0 — gist prior ASSISTANT turns to a one-line caption rather
        #   than resend the model's own verbose output verbatim; user turns keep
        #   their exact wording (that's the intent signal).
        ...
        novel_facts = [
            ...
            if not _fact_is_redundant(fact, self.recent_conversation)
        ]
        if novel_facts:
            lines = ["Known facts:"]
        ...
        if self.recent_conversation:
            ...
            for event in self.recent_conversation:
                ...
                content = (
                    _gist_assistant_turn(event.content)
        ...
        stable.extend(delta)
        stable.append("=== End Persisted Project Context ===")
