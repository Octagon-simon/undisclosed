...
def build_durable_context_for_task_lock(
    ...
    *,
    ...
    base = None if bundle.is_empty() else bundle.to_prompt(mode)

    # Semantic recall: pull the facts most relevant to the current prompt from
    # the vector memory (the piece the durable store never populated) and splice
    # them in. Skipped when the user turned semantic memory off. Fail-soft.
    facts_section = (
    ...
    if base and facts_section:
        return f"{base}\n\n{facts_section}"
    return base or facts_section or None


def _build_semantic_recall_section(
    *,
    user_key: str,
    space_id: str | None,
    project_id: str | None,
    ...
    """Render the top relevant remembered facts as a prompt fragment."""
    try:
        from app.memory import semantic_store

        facts = semantic_store.recall(
            user_key, space_id, project_id, query, k=5
        )
    except Exception:  # noqa: BLE001 — best-effort
        return None
    if not facts:
        return None
    lines = ["<remembered_facts>", "Relevant facts remembered from earlier work:"]
...
def finalize_task_lock_run_memory(
    ...
    *,
    ...
    try:
        ...
        _remember_run_outcome(
            run_context, state=state, summary=summary, final_result=final_result
        )
        return True
    ...
    *,
    ...
    try:
        ...
        semantic_store.remember(
            ...
            text,
            source="run_summary",
            extra={"run_id": getattr(run_context, "run_id", "")},
        )
    except Exception:  # noqa: BLE001 — best-effort
        logger.debug("semantic remember (run outcome) failed", exc_info=True)
