...
/** Mirrors the backend's `GovernanceMode` (`app.agent.toolkit.governance_toolkit`). */
export type GovernanceMode = 'auto' | 'ask';
...
interface AuthState {
  ...
  /** The last model the user actively selected, used to seed NEW conversations
   *  (so launch resumes their last model instead of the stock default). */
  lastUsedModel: import('./projectStore').ProjectModelSelection | null;
  /**
   * Governance gate for risky single-agent tools (terminal, file write).
   * 'auto' runs them immediately (default); 'ask' requires an explicit
   * approve/deny before each call executes. Read once per task at agent
   * assembly time, so changing this only affects tasks started afterward.
   */
  governanceMode: GovernanceMode;
  /** Semantic (long-term) memory on/off. Sent to the backend in toolkit_config. */
  memoryEnabled: boolean;
  /** Show a reasoning model's thoughts in a collapsible thinking block. */
  showThinking: boolean;
  /**
   * Feature flag for the embedded code editor (Eclipse Theia). Off by
   * default — not everyone wants an IDE, and enabling it spawns a local
   * editor server. Gates both the "Code" workspace tab and the spawn.
   */
  codeEditorEnabled: boolean;
  /** Last folder opened in the embedded editor, so returning to the Code tab
   *  reopens it instead of re-prompting for a folder. */
  codeEditorLastFolder: string | null;
  /** Width (px) of the editor's right-docked agent panel; user-resizable. */
  codeEditorDockWidth: number;
  /**
  ...
  setCodeEditorEnabled: (codeEditorEnabled: boolean) => void;
  setCodeEditorLastFolder: (codeEditorLastFolder: string | null) => void;
  setCodeEditorDockWidth: (codeEditorDockWidth: number) => void;
...
const authStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      codeEditorLastFolder: null,
      codeEditorDockWidth: 420,
      ...
      setGovernanceMode: (governanceMode) => set({ governanceMode }),
      setMemoryEnabled: (memoryEnabled) => set({ memoryEnabled }),
      setShowThinking: (showThinking) => set({ showThinking }),

      setCodeEditorEnabled: (codeEditorEnabled) => set({ codeEditorEnabled }),

      setCodeEditorLastFolder: (codeEditorLastFolder) =>
        set({ codeEditorLastFolder }),

      setCodeEditorDockWidth: (codeEditorDockWidth) =>
        set({ codeEditorDockWidth }),
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // Persist the last-used model so NEW conversations start on it after a
        // reload/relaunch instead of resetting to the stock default. Without
        // this it lives only in memory and is lost on refresh.
        lastUsedModel: state.lastUsedModel,
        governanceMode: state.governanceMode,
        memoryEnabled: state.memoryEnabled,
        showThinking: state.showThinking,
        codeEditorEnabled: state.codeEditorEnabled,
        codeEditorLastFolder: state.codeEditorLastFolder,
        codeEditorDockWidth: state.codeEditorDockWidth,
