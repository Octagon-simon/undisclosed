...
/**
 * A pending governance approval surfaced via SSE `approval_request` (see
 * `ApprovalRequestPayload` in `backend/app/service/task.py`). Cleared on
 * `approval_resolved`, whether resolved from this client or elsewhere
 * (e.g. a beckon hook posting directly to `/chat/{id}/approval`).
 */
...
export interface ChatStore {
  ...
  setActiveApproval: (taskId: string, approval: ActiveApproval | null) => void;
  addWebViewUrl: (
    taskId: string,
    webViewUrl: string,
  ...
  clearAcknowledgement: (taskId: string) => void;
...
const chatStore = (initial?: Partial<ChatStore>) =>
  createStore<ChatStore>()((set, get) => ({
    ...
    startTask: async (
      ...
      if (language === 'system') {
        try {
          // Desktop resolves the OS language over IPC; the embed (no IPC) falls
          // back to the browser's language instead of hardcoding English.
          systemLanguage =
      ...
      // Replay/share APIs live on the server side, not Brain. An embedded host
      // (mountAgentPanel) injects the proxy base at runtime and is served from a
      // PRODUCTION bundle (import.meta.env.DEV === false, VITE_BASE_URL unset),
      // so without this the playback URL became `undefined/api/...` → 404 and
      // replay streamed nothing (empty "planning" skeleton on reload). Prefer
      // the injected proxy; otherwise keep the original dev/build behavior.
      const injectedProxyBase = getConnectionConfig().proxyEndpoint;
      const serverBaseUrl = injectedProxyBase
        ? injectedProxyBase.replace(/\/+$/, '')
        : import.meta.env.DEV
      ...
      const requestBody = !type
        ? {
            ...
            // User-configured governance gate (Settings > Privacy): 'ask'
            // requires explicit approval before terminal/file-write tool
            // calls execute; 'auto' (default) runs them immediately.
            toolkit_config: {
      ...
      const ssePromise = sseTransport({
        ...
        async onmessage(event: any) {
          ...
          if (agentMessages.step === AgentStep.CONFIRMED) {
            ...
            // The Brain now echoes the turn's attaches (upload:// refs) on the
            // confirmed event, so a replayed/reloaded conversation can rebuild
            // its image/file attachments (which don't otherwise round-trip).
            const confirmedAttachRefs: string[] = Array.isArray(
              (agentMessages.data as { attaches?: unknown })?.attaches
            )
              ? ((agentMessages.data as { attaches?: string[] })
                  .attaches as string[])
              : [];
            // Rebuild attach objects from the confirmed event's refs
            // (upload:// strings) so a replay/reload restores the turn's
            // image/file attachments. Used both when creating the follow-up
            // chat store (below) and when backfilling the FIRST replayed user
            // message (which replay() seeds text-only).
            const rebuiltFromConfirmed = confirmedAttachRefs.map(
              (ref) =>
                ({
                  filePath: ref,
                  fileName: deriveAttachFileName(ref),
                  fileId: ref,
                  source: 'upload',
                }) as unknown as File
            );
            ...
            if (shouldCreateNewChat && !skipFirstConfirm) {
              ...
              if (newChatResult) {
                ...
                // Attaches for the new turn's user message. Prefer what the user
                // actually attached THIS turn (lastMessage). On a follow-up with
                // no new attachment, this must be empty -- NOT a fallback to
                // `messageAttaches`, which is the closure-captured attaches from
                // the ORIGINAL startTask (the long-lived SSE consumer never
                // re-runs). Falling back there re-attaches the first turn's image
                // to every subsequent turn, so the image "bleeds" back into the
                // chatbox and the agent keeps re-describing it. Only the very
                // first confirm (not a follow-up) should seed from messageAttaches.
                // On a LIVE turn lastMessage already carries richer attach
                // objects, so it wins; rebuiltFromConfirmed is the
                // replay/reload source (lastMessage was reconstructed empty).
                const attachesForNewMessage =
                  lastMessage?.role === 'user' && lastMessage?.attaches?.length
                    ? lastMessage.attaches
                    : rebuiltFromConfirmed.length
                      ? rebuiltFromConfirmed
                      : isFollowUpConfirm
                        ? []
                        : [
                ...
                newChatStore.getState().clearAcknowledgement(newTaskId);
              ...
              // The first user message was seeded text-only (by replay() on
              // reload, or startTask on a live first turn). If the confirmed
              // event carries attaches and that message has none yet, backfill
              // them so a reloaded conversation restores its image/file
              // attachments (they don't otherwise round-trip through playback).
              if (rebuiltFromConfirmed.length) {
                const firstUserMsg = previousChatStore.tasks[
                  currentTaskId
                ]?.messages.find(
                  (m) => m.role === 'user' && !m.attaches?.length
                );
                if (firstUserMsg) {
                  previousChatStore.updateMessage(
                    currentTaskId,
                    firstUserMsg.id,
                    { ...firstUserMsg, attaches: rebuiltFromConfirmed }
                  );
                }
              }
            }
          ...
          // One-time turn-start acknowledgement ("I'm on it") — a presence
          // line generated by a decoupled model call on the backend. Not part
          // of the agent's answer/history; just shown live above the thinking.
          if (agentMessages.step === AgentStep.ACKNOWLEDGE) {
            const text =
              (agentMessages.data as { acknowledgement?: string })
                ?.acknowledgement ?? '';
            if (text) setAcknowledgement(currentTaskId, text);
            return;
          }
          ...
          if (agentMessages.step === AgentStep.END) {
            ...
            // Promote any leftover non-terminal subtasks to COMPLETED on a real
            // completion. The agent's last todo_state can leave the final item
            // running/pending, so the pinned plan showed "incomplete" until a
            // reload ran polishCompletedHistoryTask. Do the same fixup LIVE so
            // the plan reads complete immediately (failures stay visible; stops
            // are left as-is).
            if (!wasStoppedByUser) {
              const finished = tasks[currentTaskId];
              const promote = (s?: TaskInfo['status']): TaskInfo['status'] =>
                s === TaskStatus.COMPLETED || s === TaskStatus.FAILED
                  ? s
                  : TaskStatus.COMPLETED;
              if (finished?.taskInfo?.length) {
                setTaskInfo(
                  currentTaskId,
                  finished.taskInfo.map((t) => ({ ...t, status: promote(t.status) }))
                );
              }
              if (finished?.taskRunning?.length) {
                setTaskRunning(
                  currentTaskId,
                  finished.taskRunning.map((t) => ({
                    ...t,
                    status: promote(t.status),
                  }))
                );
              }
              if (finished?.taskAssigning?.length) {
                setTaskAssigning(
                  currentTaskId,
                  finished.taskAssigning.map((a) => ({
                    ...a,
                    tasks: (a.tasks ?? []).map((t) => ({
                      ...t,
                      status: promote(t.status),
          ...
          if (agentMessages.step === AgentStep.APPROVAL_REQUEST) {
            const data = agentMessages.data as {
              approval_id: string;
              tool_name: string;
              detail?: string;
              agent_name?: string | null;
          ...
          if (agentMessages.step === AgentStep.APPROVAL_RESOLVED) {
            const data = agentMessages.data as {
              approval_id: string;
              decision: string;
              decided_by?: string | null;
            };
            // Only react if this is the approval we're currently showing --
            // an approval that resolved after we'd already moved on (e.g. a
            // duplicate/late event) shouldn't clear a newer pending one.
            const current = tasks[currentTaskId]?.activeApproval;
            if (current && current.approval_id === data.approval_id) {
              setActiveApproval(currentTaskId, null);
              // Governance can gate many tools in a row; appending a fresh
              // outcome message per approval floods the chat with stacked
              // '"terminal" was approved by user.' lines. Keep a single,
              // in-place record per task (stable id) that updates to the most
              // recent decision instead of accumulating.
              const outcomeId = `approval-outcome-${currentTaskId}`;
              // Phrase per decision so it reads as natural English. 'timeout'
              // is a noun ("was timeout by timeout" is wrong); approved/denied
              // are past participles. Only name the decider when it's someone
              // other than the current user / the timeout itself.
              const toolLabel = `"${current.tool_name}"`;
              const by =
                data.decided_by &&
                data.decided_by !== 'user' &&
                data.decided_by !== 'timeout'
                  ? ` by ${data.decided_by}`
                  : '';
              const outcomeContent =
                data.decision === 'timeout'
                  ? `${toolLabel} timed out waiting for approval.`
                  : `${toolLabel} was ${data.decision}${by}.`;
              const alreadyShown = tasks[currentTaskId]?.messages?.some(
                (m: Message) => m.id === outcomeId
              );
              const outcomeMessage: Message = {
                id: outcomeId,
                role: 'agent',
                content: outcomeContent,
                step: AgentStep.APPROVAL_OUTCOME,
                decision: data.decision,
              };
              if (alreadyShown) {
                updateMessage(currentTaskId, outcomeId, outcomeMessage);
              } else {
                addMessages(currentTaskId, outcomeMessage);
              }
            }
            return;
          }
    ...
    addMessages(taskId, message) {
      ...
      try {
        ...
        console.warn('Error in addMessages persist hook:', persistErr);
      }
    },
    ...
    setActiveApproval(taskId, approval) {
      set((state) => ({
        ...state,
        tasks: {
          ...state.tasks,
    ...
    setAcknowledgement: (taskId, text) => {
      set((state) => {
        const task = state.tasks[taskId];
        if (!task) return state;
        return {
          ...
          tasks: {
            ...state.tasks,
            [taskId]: { ...task, acknowledgement: text },
          },
        };
      });
    },
    clearAcknowledgement: (taskId) => {
      set((state) => {
        const task = state.tasks[taskId];
        if (!task || !task.acknowledgement) return state;
        return {
          ...state,
          tasks: {
            ...state.tasks,
            [taskId]: { ...task, acknowledgement: '' },
          },
        };
      });
    },
