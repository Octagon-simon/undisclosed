...
const projectStore = create<ProjectStore>()((set, get) => ({
    ...
    const newProject: Project = {
      ...
      metadata: {
        ...
        ...(description === 'Auto-created project'
          ...
          : {}),
        // Seed a NEW conversation with the user's last-used model so launch
        // resumes it instead of the stock default. Replays keep their own; an
        // explicit options.metadata.modelSelection still wins (spread below).
        ...(type !== ProjectType.REPLAY && getAuthStore().lastUsedModel
          ? { modelSelection: getAuthStore().lastUsedModel ?? undefined }
          : {}),
        ...options?.metadata,
      },
    };
  ...
  setProjectModel: (
    ...
    // Remember this as the user's last-used model so NEW conversations start on
    // it (persisted in authStore) instead of resetting to the stock default.
    getAuthStore().setLastUsedModel(modelSelection);
