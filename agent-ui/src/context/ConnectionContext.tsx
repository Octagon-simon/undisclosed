  ...
  useEffect(() => {
    ...
    const resolveEndpoint = async () => {
      ...
      if (envEndpoint && typeof envEndpoint === 'string') {
        ...
        // An embedder (mountAgentPanel) may have already injected the endpoint;
        // use it and stay quiet. Only warn when nothing is configured.
        const injected = getConnectionConfig().brainEndpoint;
        if (injected) {
          resolvedEndpoint = injected.replace(/\/$/, '');
        } else {
          console.error(
