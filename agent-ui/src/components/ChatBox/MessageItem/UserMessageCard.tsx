...
import { resolveAttachmentImageUrl } from '@/lib/attachmentUrl';
              ...
              return (
                <>
                  {visibleFiles.map((file) => {
                    // Image attaches render as an inline thumbnail (served by the
                    // Brain in web/embed, where there is no electronAPI). Falls
                    // back to the file chip when it isn't a servable image.
                    const imageUrl = resolveAttachmentImageUrl(file as any);
                    if (imageUrl) {
                      return (
                        <div
                          key={'attache-' + file.fileName}
                          className="relative box-border h-16 w-16 cursor-pointer overflow-hidden rounded-lg border border-solid border-ds-border-neutral-subtle-default bg-ds-bg-neutral-default-default"
                          title={file.fileName}
                          onClick={(e) => {
                            e.stopPropagation();
                    ...
                    return (
                      <div
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          // Desktop reveals in the OS file manager; the embed
                          // (no IPC) opens the file in the editor instead.
