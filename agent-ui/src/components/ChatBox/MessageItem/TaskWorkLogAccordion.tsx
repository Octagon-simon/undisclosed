...
import { ActivityTimeline } from '@/components/ChatBox/MessageItem/ActivityTraceCard';
import {
  InlineMessageRow,
  type MessageItem,
  type TimelineItem,
...
// Re-exported for existing consumers/tests that imported these types from
// this module before they were split out into `workLogTimeline`.
export type { MessageItem, TimelineItem, ToolItem };
 ...
 * instead of a static "Undisclosed Agent" tag.
 *
...
function looksLikeNarration(raw: string): boolean {
  ...
  // Toolkit parameter-dump messages (e.g. FileToolkit's "write content to file:
  // /path with encoding: None and use_latex: False") read like a sentence but
  // are redundant with the humanized activity row — hide them.
  if (/\b(use_latex|with encoding)\s*:/i.test(s)) return false;
  if (/^write content to file\s*:/i.test(s)) return false;
...
export function buildAgentBlocks(
  ...
  try {
    const sample = tagged.slice(-3).map((t) => ({ step: t.entry.step, msg: String(t.entry.data?.message || '').slice(0, 80) }));
    // Trace: help debug live work-log sequence composition
    // eslint-disable-next-line no-console
    console.debug('[WORKLOG MSG][BUILD_BLOCKS]', { count: tagged.length, isSingleAgent, sample });
  } catch {}
        ...
        {open ? (
          ...
          >
            <div className="flex flex-col gap-2 py-1">
              {/* Preparation blocks (agent registration, browser session
                  cloning) are setup noise, not user-visible "activity" — the
                  classifier would otherwise mislabel e.g. clone_for_new_session
                  as "Browsed ChatAgent(...)". Keep them on the plain row list. */}
              {isActivityTraceEnabled() && block.kind === 'action' ? (
                <ActivityTimeline
                  items={block.items}
                  running={taskRunning && block.status === 'running'}
                />
              ) : (
...
const AgentGroupRow = memo(function AgentGroupRow({
  ...
  // static "Undisclosed Agent" label. Fall back to the static label only when no
        ...
        {open ? (
          ...
          >
            <div className="flex flex-col gap-2 py-1 pl-6">
              {isActivityTraceEnabled() ? (
                <ActivityTimeline
                  items={group.items}
                  running={taskRunning && group.status === 'running'}
                />
              ) : (
