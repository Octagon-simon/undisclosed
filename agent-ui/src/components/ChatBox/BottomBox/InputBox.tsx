...
export interface InputboxProps {
  ...
  onSend?: () => void;
  /** True while a task is actively running. When set (and the input is empty),
   *  the send button becomes a Stop button that calls `onStop`. */
  isRunning?: boolean;
  /** Stop the active task (wired to the composer's Stop button). */
  onStop?: () => void;
          ...
          {isRunning && value.trim().length === 0 ? (
            <TooltipSimple content="Stop task" side="top">
              <Button
                ...
                onClick={() => onStop?.()}
              >
                {/* Inline color+fill: the panel is native in Theia's DOM where
                    a global `fill`/color rule overrode the class + the
                    `fill="currentColor"` attribute, leaving the square black.
                    Inline style wins, guaranteeing a red stop glyph. */}
                <Square
                  size={14}
