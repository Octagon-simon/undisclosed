...
export default function ChatBox(): JSX.Element {
  ...
  // Consume a one-shot composer prefill from the welcome/empty state (embed).
  const composerDraft = useComposerDraft((s) => s.draft);
  const clearComposerDraft = useComposerDraft((s) => s.clearDraft);
  useEffect(() => {
  ...
  const handleSend = async (
    ...
    const isTaskBusy =
      ...
      (!canSendWhileBusy && task.isTakeControl) ||
    ...
    try {
      if (requiresHumanReply) {
        ...
        // 4. Single-agent task currently running - follow-up is injected
        //    into the live agent by the backend (POST /chat/{id})
        ...
        >
          {hasAnyMessages ? (
            ...
            <div className="mx-auto flex min-h-full w-full max-w-[600px] flex-col">
              <ConversationWelcome onPick={setMessage} />
