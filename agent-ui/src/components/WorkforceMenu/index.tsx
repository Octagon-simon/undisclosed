    ...
    <div className="relative z-50 flex h-12 items-center justify-center pt-2">
      <div className="w-full">
        <div className="relative flex h-full w-full flex-row items-center justify-center">
          ...
          {/* Code editor toggle (feature-flagged) */}
          {codeEditorEnabled && (
            <div className="absolute left-2 flex items-center pb-2">
              <Button
                variant={
                  getCurrentTask()?.activeWorkspace === 'code'
                    ? 'primary'
                    : 'ghost'
                }
                size="sm"
                onClick={() => {
                  if (!chatStore?.activeTaskId) return;
                  const current = getCurrentTask()?.activeWorkspace;
                  chatStore.setActiveWorkspace(
                    chatStore.activeTaskId,
                    current === 'code' ? 'workflow' : 'code'
                  );
                  host?.electronAPI?.hideAllWebview?.();
                }}
                className="gap-1.5"
              >
                <CodeXml size={16} />
                <span className="text-label-sm font-semibold">Code</span>
              </Button>
            </div>
          )}
