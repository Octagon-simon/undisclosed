...
export default function SettingPrivacy() {
  ...
  const governanceMode = useAuthStore((state) => state.governanceMode);
  const setGovernanceMode = useAuthStore((state) => state.setGovernanceMode);
  const codeEditorEnabled = useAuthStore((state) => state.codeEditorEnabled);
  const setCodeEditorEnabled = useAuthStore(
    (state) => state.setCodeEditorEnabled
  );
  ...
  const handleToggleGovernanceMode = (checked: boolean) => {
    setGovernanceMode(checked ? 'ask' : 'auto');
  };
      ...
      <div className="mb-8 flex flex-col gap-6">
        ...
        <div className="rounded-2xl bg-ds-bg-neutral-default-default px-6 py-4">
          <div className="flex items-center justify-between gap-md">
            <div className="flex flex-col gap-2">
              <div className="text-body-base font-bold text-ds-text-neutral-default-default">
                {t('setting.require-approval-for-risky-actions')}
              </div>
              <div className="text-body-sm font-normal text-ds-text-neutral-default-default">
                {t('setting.require-approval-for-risky-actions-description')}
              </div>
            </div>
            <div className="flex items-center justify-center">
              <Switch
                checked={governanceMode === 'ask'}
                onCheckedChange={handleToggleGovernanceMode}
              />
            </div>
          </div>
        </div>

        {/* Code Editor (beta) Section */}
        <div className="rounded-2xl bg-ds-bg-neutral-default-default px-6 py-4">
          <div className="flex items-center justify-between gap-md">
            <div className="flex flex-col gap-2">
              <div className="text-body-base font-bold text-ds-text-neutral-default-default">
                {t('setting.code-editor', {
                  defaultValue: 'Code Editor (beta)',
                })}
              </div>
              <div className="text-body-sm font-normal text-ds-text-neutral-default-default">
                {t('setting.code-editor-description', {
                  defaultValue:
                    'Show an embedded local code editor for your projects. Runs entirely on your machine.',
                })}
              </div>
            </div>
            <div className="flex items-center justify-center">
