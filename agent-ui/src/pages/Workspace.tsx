...
import CodeWorkspaceView from '@/components/CodeAgentWorkspace/CodeWorkspaceView';
import { WorkspaceDispatch } from '@/components/Dispatch';
import Folder from '@/components/Folder';
import ManageTabBar from '@/components/ManageView/ManageTabBar';
import { isManageTab } from '@/components/ManageView/manageTabs';
...
// One-shot per renderer boot: in editor-first mode, land on the code editor
// (the primary view) instead of the workforce tab. Module-scoped so sidebar/
// workspace remounts within a session don't re-trigger it.
let didAttemptEditorFirstBoot = false;
...
export default function WorkspacePage() {
  ...
  // Editor-first boot: default to the code editor as the primary view. Reopens
  // the last-visited project (if any) so the agent panel is populated, then
  // lands on the `code` tab. One-shot; only from the pristine boot state so
  // deliberately navigating elsewhere later is never hijacked. The sidebar's
  // own resume effect stands down when the editor is enabled (see below).
  useEffect(() => {
    if (didAttemptEditorFirstBoot) return;
    if (!codeEditorEnabled) return;
    didAttemptEditorFirstBoot = true;
    if (usePageTabStore.getState().activeWorkspaceTab !== 'workforce') return;

    const spaceId = useSpaceStore.getState().activeSpaceId;
    const lastVisitedId = spaceId
      ? useSpaceStore.getState().lastVisitedProjectBySpace[spaceId]
      : undefined;
    if (lastVisitedId) {
      const projectStore = useProjectRuntimeStore.getState();
      projectStore.setActiveProject(lastVisitedId);
      void ensureProjectLoaded(projectStore, lastVisitedId);
    }
    setActiveWorkspaceTab('code');
    // One-shot boot effect: later value changes must not re-trigger it.
  ...
  const renderActiveWorkspaceTab = () => {
    switch (activeWorkspaceTab) {
      ...
      case 'code':
        // The editor is rendered persistently below (so its iframe survives
        // tab hops without reloading); nothing to render inline here.
        return null;
      case 'inbox':
        return (
          <div className={mainPanelContentClass}>
    ...
    <ReactFlowProvider>
      <div className="flex h-full min-h-0 flex-row overflow-hidden px-1 pb-1 pt-10">
        <div
          ...
          className="relative h-full min-h-0 w-full min-w-0 flex-1 rounded-2xl bg-ds-bg-neutral-subtle-default"
        >
          <ResizablePanelGroup
          ...
          >
            ...
            >
              ...
              >
                <div className={cn(mainPanelShellClass, 'relative')}>
                  {codeEditorEnabled && isManageTab(activeWorkspaceTab) ? (
                    // Editor-first: the three space-scoped surfaces
                    // (Context/Scheduled/Dispatch) render as one grouped
                    // "Manage" view with an internal tab bar.
          ...
          {/* Persistent editor overlay: in editor-first mode the code editor is
              the PRIMARY surface, so it covers the whole shell (both the left
              nav and the main panel) on the code tab. Always mounted once
              enabled so the code-server iframe never reloads on tab hops; the
              underlying sidebar stays mounted (its effects keep running) simply
              hidden behind this. */}
          {codeEditorEnabled && (
            <div
              className="absolute inset-0 z-20 bg-ds-bg-neutral-subtle-default"
              style={{
                display: activeWorkspaceTab === 'code' ? 'block' : 'none',
              }}
            >
              <CodeWorkspaceView active={activeWorkspaceTab === 'code'} />
            </div>
          )}
        </div>
