...
/**
 * POST with an AbortSignal timeout so the request can never sit in
 * "pending" forever. Used for long-running operations such as model
 * validation where an unresponsive provider would otherwise leave the
 * UI stuck with no feedback.
 */
export const fetchPostWithTimeout = (
  url: string,
  data?: any,
  headers?: any,
  timeoutMs = 100000
...
export async function getProxyBaseURL() {
  // An embedder (e.g. mountAgentPanel) can inject the proxy base URL at runtime,
  // the same way brainEndpoint is injected. Takes precedence over build envs.
  const injectedProxy = getConnectionConfig().proxyEndpoint;
  if (injectedProxy) {
    return injectedProxy.replace(/\/$/, '');
  }
