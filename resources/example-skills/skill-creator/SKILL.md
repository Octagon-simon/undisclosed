...
**Install location (IMPORTANT):** New skills MUST be created in the user skills
directory `~/.undisclosed/skills` so the agent runtime can discover and load
them. Use that as the `--path`:

```bash
python scripts/init_skill.py <skill-name> --path ~/.undisclosed/skills
```

This creates `~/.undisclosed/skills/<skill-name>/`. Do NOT create skills in any
other or legacy skills directory — only `~/.undisclosed/skills` is loaded by the
runtime. For a skill that should only exist inside the current project, use
`<project>/.undisclosed/skills` instead.
